...
// Implementation lives in services/idl-fetch.ts so the FlowAuthorAgent Durable
// Object shares the same resolution path + warm-isolate memo.
  ...
  server.tool(
    ...
    async ({ projectId, address, cluster }) => {
      try {
        ...
        const { rpcUrl } = resolveSolanaRpcUrl({ network, env })
        ...
        try {
          ...
          return {
            isError: true,
            content: [{ type: 'text', text: `RPC error on ${rpcUrlHost(rpcUrl)}: ${rpcErr.message}` }],
          }
        }
        ...
        if (!rawInfo) {
          return {
            isError: true,
            content: [{ type: 'text', text: `Account \`${address}\` not found on ${network}.` }],
          }
        }
        ...
        const lines: string[] = [
          `**Account: \`${address}\`**`,
          '',
          `Type: ${accountDef ? `\`${accountDef.name}\`` : '_Unknown (no IDL match)_'}`,
          `Owner: \`${rawInfo.owner}\``,
          `Balance: ${(rawInfo.lamports / 1_000_000_000).toFixed(9).replace(/\.?0+$/, '')} SOL`,
          `Cluster: ${network} | Slot: ${rawInfo.slot.toLocaleString()}`,
        ]

        if (parseError) {
          lines.push('', `⚠️ Parse error: ${parseError}`)
        }

        if (parsedData !== null) {
          lines.push('', '**Decoded Fields:**', '```json', JSON.stringify(parsedData, null, 2), '```')
        } else if (!parseError) {
